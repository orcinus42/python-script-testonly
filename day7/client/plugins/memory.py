...
def monitor():
	...
	if status != 0: #cmd exec error
		...
		if monitor_dic['SwapUsage'] == 'precentage':
			value_dic['SwapUsage_p'] == 100 - int(value_dic['SwapFree']) * 100 / int(value_dic['SwapTotal']) #real SwapUsage value
		...
		if monitor_dic['MemUsage'] == 'percentage':
			value_dic['MemUsage_p'] = int(MemUsage) * 100 / int(value_dic['MemTotal']) #real MemUsage value
